...
export default class CodePlugin extends Plugin {
  ...
  serialize = (
    ...
    if (object.object === 'mark') {
      switch (object.type) {
        case CODE:
          return <code className="ory-plugins-content-slate-next-code">{children}</code>
      }
  ...
  renderMark = props => {
    ...
    switch (mark.type) {
      case CODE:
        return <code {...attributes} className="ory-plugins-content-slate-next-code">{children}</code>
    }
  }
