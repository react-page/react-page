 ...
 */
export type ControlsDef<DataT extends Data> = AutoformControlsDef<DataT> | CustomControlsDef<DataT>;
...
export type SlateDataPluginDefinition<T extends Data> = SlateNodeBasePluginDefinition<T> & {
    dataMatches: (data: T) => boolean;
    /**
     * if defined these properties will be removed from data when plugin gets disabled
     */
    properties?: T extends Record<string, unknown> ? Array<keyof T> : Array<DataTAny>;
};
export type SlateCustomPluginDefinition<T extends Data> = SlateBasePluginDefinition<T> & Record<string, unknown>;
...
type SlateComponentPluginComponent<T extends Data> = keyof JSX.IntrinsicElements | React.ComponentType<NoInfer<{
    /**
...
export type SlateComponentPluginDefinition<T extends Data> = SlateNodeBasePluginDefinition<T> & {
    /**
...
export type SlatePluginDefinition<T extends DataTType = DataTAny> = (SlateComponentPluginDefinition<T> & {
